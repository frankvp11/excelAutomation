...
from helper2 import write_json, write_txt
from text_based import read_txt
...
def existing_client():
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            client_name.handle_event(event, SCREEN)
        if is_in_text:
            found_client_text = get_font(20).render("Found Client!", True, 'white')
            found_client_rect = found_client_text.get_rect(center=(640, 300))
            SCREEN.blit(found_client_text, found_client_rect)
        client_name.update()
        client_name.draw(SCREEN)
